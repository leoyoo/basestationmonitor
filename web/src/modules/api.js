...
export default {
    ...
    getStationDataByDay(stationId, seqId, startHour, endHour) {

        // let start = `${date} 00:00:00`;
        // let end = `${date} 02:59:59`;
        return axios.get(
            `${storage.getServiceUrl()}/api/stationdata?stationid=${stationId}&seqid=${seqId}&start=${startHour}&end=${endHour}&count=100`, {
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                    "Access-Control-Allow-Origin": "*",
